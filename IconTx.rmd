...
```{r}
raw_data <- read.csv(file='networkiconTX.txt',header=F,sep=' ')
...
```
```{r}
Total.circulation.amount <-473406688*(10^18)
...
xlab="Frequencies",
ylab = "Number of pairs", col="lightblue",freq = FALSE, breaks = 300, xlim = c(0, quantile(x, 0.99)))
...
```{r}
BuyerData$percentage<-(BuyerData$TokenAmount/BuyerData$Volume)*100
BuyerData<- subset(BuyerData,percentage<100)
BuyerData
```
```{r}
cor.test(BuyerData$TokenAmount,BuyerData$Volume, method = "pearson")
```

```{r}
linearMod <- lm(TokenAmount ~ Volume, data=BuyerData)  # build linear regression model on full data
linearMod
```
```{r}
summary(linearMod)
...
beta.estimate <- modelCoeffs["Volume", "Estimate"]  # get beta estimate for speed
std.error <- modelCoeffs["Volume", "Std. Error"]  # get std.error for speed
